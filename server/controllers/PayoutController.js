...
export const getSellerEarningsSummary = async (req, res) => {
    ...
    try {
        const result = await pool.query(`
            SELECT 
                COALESCE(SUM(CASE WHEN LOWER(os.payout_status) = 'pending' AND o.order_status = 'Delivered' THEN os.seller_earnings ELSE 0 END), 0) as withdrawable_balance,
                COALESCE(SUM(CASE WHEN LOWER(os.payout_status) = 'pending' AND o.order_status != 'Delivered' THEN os.seller_earnings ELSE 0 END), 0) as pending_delivery,
                COALESCE(SUM(CASE WHEN LOWER(os.payout_status) = 'processing' THEN os.seller_earnings ELSE 0 END), 0) as processing_payouts,
                COALESCE(SUM(CASE WHEN LOWER(os.payout_status) = 'paid' THEN os.seller_earnings ELSE 0 END), 0) as paid_earnings,
                COALESCE(SUM(os.seller_earnings), 0) as total_earnings
...
// Get all pending payouts for a seller (eligible for payout)
export const getPendingCommissions = async (req, res) => {
    const { sellerId } = req.params;

    if (req.user.type === 'seller' && req.user.id !== sellerId) {
        return res.status(403).json({ success: false, message: 'Unauthorized access to payouts' });
    }

    try {
        const result = await pool.query(`
            SELECT os.*, o.placed_at as created_at 
            FROM order_sellers os
            JOIN orders o ON os.order_id = o.order_id
            WHERE os.seller_id = $1 
...
export const requestPayout = async (req, res) => {
    ...
    try {
        ...
        // 3. Link and mark only 'Delivered' pending order sellers as 'processing'
        await client.query(`
...
export const updatePayoutStatus = async (req, res) => {
    ...
    try {
        ...
        `, [status, finalAdminId, transaction_ref, notes, payout_id]);
         ...
         // 3. Update order_sellers based on status - USING payout_id LINK
         if (status === 'Paid') {
             // Mark order_sellers as Paid
             ...
             // Revert order_sellers to 'pending'
             await client.query(`
                  UPDATE order_sellers 
...
export const initiatePayout = async (req, res) => {
    ...
    try {
        ...
        // 2. Update the order_sellers to 'Paid'
        await client.query(`
            UPDATE order_sellers 
            SET payout_status = 'Paid' 
            WHERE seller_id = $1 
            AND (payout_status = 'Pending' OR payout_status = 'Processing')
            AND created_at >= $2 
            AND created_at <= $3
...
export const processAutoPayout = async (orderId) => {
    ...
    try {
        ...
        // 1. Get all pending payouts for this order
        const commissionsRes = await client.query(`
            ...
            FROM order_sellers 
            WHERE order_id = $1 AND LOWER(payout_status) = 'pending'
            GROUP BY seller_id
        `, [orderId]);
        ...
        for (const row of commissionsRes.rows) {
            ...
            // 3. Update order_sellers to 'Paid'
            await client.query(`
                UPDATE order_sellers 
                SET payout_status = 'Paid' 
                WHERE order_id = $1 AND seller_id = $2 AND LOWER(payout_status) = 'pending'
            `, [orderId, seller_id]);
