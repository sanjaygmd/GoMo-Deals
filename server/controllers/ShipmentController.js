...
export const pushOrderToShiprocket = async (orderId, client = pool) => {
    try {
        ...
        // [ADDRESS FIX] Customer address must be at least 10+ chars and contain valid separators (relaxing regex to support #, (), and ;)
        let customerAddress = (order.address_line_1 || '').trim().replace(/[^\w\s\-,./#():;]/g, '');
        if (customerAddress.length < 10) {
            customerAddress = `Street No. 1, ${customerAddress}`;
        }
        ...
        const srOrderItems = items.map(item => ({
            ...
            units: item.quantity,
            selling_price: Math.max(1, Number(item.unit_price)) // Price cannot be 0
        }));
        ...
        // Generate a slightly shorter unique order ID if possible, but keep UUID part
        ...
        const srPayload = {
            ...
            pickup_location: (pickupLocation.location_name || "").trim(),
            ...
            length: Math.max(1, ...items.map(i => {
                const len = Number(i.length);
                return (isNaN(len) || len <= 0) ? 10 : len;
            })),
            breadth: Math.max(1, ...items.map(i => {
                const brd = Number(i.breadth);
                return (isNaN(brd) || brd <= 0) ? 10 : brd;
            })),
            height: Math.max(1, ...items.map(i => {
                const hgt = Number(i.height);
                return (isNaN(hgt) || hgt <= 0) ? 10 : hgt;
            })),
