...
export const getCustomerNotifications = async (req, res) => {
    ...
    // Ownership Check
    if (req.user.id !== customerId && req.user.type !== 'admin' && req.user.type !== 'super_admin') {
...
export const markAsRead = async (req, res) => {
    ...
    try {
        // Ownership Check
        const noteCheck = await pool.query("SELECT customer_id, seller_id, admin_id FROM notifications WHERE notification_id = $1", [notificationId]);
        ...
        const isAuthorized = 
            req.user.id === note.customer_id || 
            req.user.id === note.seller_id || 
            req.user.id === note.admin_id ||
            req.user.type === 'admin' || 
