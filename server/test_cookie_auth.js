...
async function main() {
    try {
        ...
        // Make the HTTP request to the running server using Cookie instead of Authorization header
        console.log("Making GET request to /api/admin/returns with Cookie...");
        const response = await axios.get('http://localhost:3000/api/admin/returns', {
            headers: {
                'Cookie': `admin_token=${rawToken}`,
