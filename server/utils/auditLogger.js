 ...
 */
export const logAction = async (req, action, payload = {}, dbClient = null) => {
    try {
        const user = req.user;
        ...
        // Use provided dbClient (transaction) or default pool
        ...
        console.log(`[AUDIT] Action '${action}' recorded by ${user.email} ${dbClient ? '(Transaction)' : ''}`);
    } catch (err) {
