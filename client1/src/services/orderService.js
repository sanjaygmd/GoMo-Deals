...
export const sendOrderConfirmationEmail = async (emailData) => {
    try {
        const response = await api.post(`/orders/send-confirmation`, emailData);
        return response.data;
    } catch (error) {
        // Non-critical: silently fail email sends
        console.warn('[EMAIL] Could not send order confirmation:', error);
